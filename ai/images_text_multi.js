...
export default async function get_text_image(req, res, send_api_error) {
    ...
    try {
        ...
        } else if (req.url.includes('/exalink')) {
            // Lấy query string từ URL
            const { url, randomNumber } = req.body;  // Lấy dữ liệu JSON từ req.body
            // Xử lý dữ liệu JSON
            // console.log(voice_data);
            const ketqua = await exalink(url);
...
function text_speech_embed(voice_data, randomNumber) {
    console.log(randomNumber);
    const pcmArray = JSON.parse(voice_data.data.array);
    // Bước 1: Lấy sampling_rate và PCM array từ JSON
    const samplingRate = voice_data.data.sampling_rate; // Lấy sampling_rate
